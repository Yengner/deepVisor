...
const Marketing: React.FC<ECommerceProps> = ({ campaignInsights, accounts, currentAccount, userId }) => {
  ...
  const handleAccountChange = (adAccountId: string, campaignId: string) => {
    setSelectedAdAccount(adAccountId);
    const campaign = campaignInsights.find((c) => c.campaign_id === campaignId) || null;
    setSelectedCampaign(campaign);
  };

  const costPerLead = Number(
    Array.isArray(selectedCampaign?.cost_per_action)
      ? selectedCampaign?.cost_per_action.find((item) => item?.action_type === 'lead')?.value
      : 0
  ).toLocaleString();
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats title="Total Leads" total={`${selectedCampaign?.leads || 0}`} rate="0.43%" levelUp>
          <svg
        ...
        <CardDataStats title="Total Spend" total={`$${selectedCampaign?.spend || 0}`} rate="4.35%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="20"
            height="22"
        ...
        <CardDataStats title="Total Impressions" total={`${Number(selectedCampaign?.impressions || 0).toLocaleString()}`} rate="0.95%" levelDown>
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
